...
MetalOperations::MetalOperations(MTL::Device *device)
{
    ...
    {
        std::string buf("Did not find Metal library 'ops.metallib'. Error: ");
        buf.append(error->description()->utf8String());
    ...
    for (size_t i = 0; i < fnNames->count(); i++)
    {
        ...
        {
            std::string buf("Failed to created pipeline state object for ");
            buf.append(name_utf8);
            buf.append(", error ");
            buf.append(error->description()->utf8String());
            throw std::runtime_error(buf);
        }
