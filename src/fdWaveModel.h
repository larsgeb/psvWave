 ...
 *
 * On x86_64 systems it is typically fastest to use double, as this is the native precision. Double has about a 10% performance gain over floats as
 ...
 *
 * This class contains everything needed to do finite difference wave forward and adjoint modelling.  It contains the entire experimental
 * parameters as fields, which are loaded at runtime from the supplied .ini file. The class contains all necessary functions to perform FWI, but lacks
...
public:
     ...
     * which leads to a slight performance decrease compared to hardcoding.
     *
     * @param configuration_file_relative_path Relative path to the configuration .ini file. This file should contain all the fields needed for
     * simulation. Arbitrary defaults are hardcoded into the binary as backup within the parse_configuration() method.
     *
     */
    ...
    // ---- METHODS ----
    /** \brief Method that parses .ini configuration file. Only used in fdWaveModel().
     *
     * @param configuration_file_relative_path Relative path to the configuration .ini file.
     */
     ...
     * @param store_fields Boolean to control storage of wavefields. If storage is not required (i.e. no adjoint modeling), forward simulation
     * should be faster without storage.
     ...
     *
     * This method writes out the synthetic seismograms to a plaintext file. Allows one to e.g. subsequently import these files as observed
     * seismograms later using load_receivers(). Every shot generates a separate ux and uz receiver file (rtf_ux/rtf_uz), with every receiver being a
     * single line in these files.
     *
     */
     ...
     *
     * This method loads receiver data from observed_data_folder folder into the object. The data has to exactly match the set-up, and be named
     * according to component and shot (as generated by write_receivers() ).
     *
     ...
     *
     * This method takes the kernels (lambda, mu, rho) as originally calculated on the Lamé's parameter set and maps them to the velocity parameter
     * set (vp, vs, rho).
     *
     */
     ...
     *
     * This method updates Lamé's parameters of the current model based on the velocity parameters of the current model. Typically has to be done
     * every time after updating velocity.
     *
     */
     ...
     *
     * This methods loads any appropriate model (expressed in density, P-wave velocity, and S-wave velocity) into the class and updates the Lamé
     * fields accordingly.
     *
     ...
     *
     * This method performs all necessary steps in FWI; forward modelling, misfit calculation adjoint source calculation, adjoint modelling and
     * kernel projection. Delegates to run_model(bool verbose, bool simulate_adjoint).
     *
     * @param verbose Boolean controlling the verbosity of the method.
     */
    void run_model(bool verbose);

    /** \brief Method to perform all steps necessary for FWI, with additional control over adjoint simulation.
     *
     * This method performs all necessary steps in FWI; forward modelling, misfit calculation and optionally adjoint source calculation, adjoint
     * modelling and kernel projection.
     *
     * @param verbose Boolean controlling the verbosity of the method.
     * @param simulate_adjoint Boolean controlling the execution of the adjoint simulation and kernel computation.
     */
    void run_model(bool verbose, bool simulate_adjoint);

    /** \brief Method to reset all Lamé sensitivity kernels to zero.
     *
     * This method resets all sensitivity kernels to zero. Essential before performing new adjoint simulations, as otherwise the kernels of
     * subsequent simulations would stack.
     *
     */
    void reset_kernels();
 ...
 *
 * Parses any string in the format {a, b, c, d, ...} to a vector of int, float, double. Only types that can be cast to floats are supported for now.
 * This is due to the usage of strtof(); no template variant is used as of yet. The input string is allowed to be trailed by a comment leading with
 * a semicolon. Items are appended to the passed vector.
 *
 * @tparam T Arbitrary numeric type that has a cast to and from float.
 * @param string_to_parse Input string of the form "{<item1>, <item2>, <item3>, ... } ; comment ".
 * @param destination_vector Pointer to an (not necessarily empty) vector of suitable type.
 */
template<class T>
void parse_string_to_vector(std::basic_string<char> string_to_parse, std::vector<T> *destination_vector);

/** \brief Function to parse strings containing 2d integer lists to std::vector<std::vector<int>>. Sublists do not need to be of the same length.
 *
 * @param string_to_parse Input string of the form "{{<item1.1>, <item1.2>, <item1.3>, ...}, {<item2.1>, <item2.2>, ...}, ... } ; comment ".
 * @param destination_vector Pointer to an (not necessarily empty) std::vector<std::vector<int>>.
 */
...
/** \brief Method to the compute cross correlation between two signals up to a maximum delay.
 *
 * @param signal1 Pointer to first signal.
 * @param signal2 Pointer to second signal.
 * @param r Pointer to cross-correlation array. Should be have memory allocated twice the max_delay size.
 * @param length Size of both signal1 and signal2.
 * @param max_delay Maximum delay for which to compute the cross correlation.
 */
void cross_correlate(const real_simulation *signal1, const real_simulation *signal2, real_simulation *r, int length, int max_delay = 1000);
