...
int main() {
    ...
    const arma::uword nx_domain = 200;
    const arma::uword nz_domain = 200;
    const arma::uword np_boundary = 20;
    ...
    // Static fields
    double rho = 1500.0;
    const arma::Mat<double> vp = (2000.0 * 2000.0) * arma::ones(nx, nz);
    ...
    taper = arma::exp(-arma::square(0.015 * (20 - taper)));
...
//    std::cout << taper;
    ...
    for (int it = 0; it < nt; ++it) {
        ...
        px(50 + np_boundary, 50) += 0.5 * dt * source[it];
        pz(50 + np_boundary, 50) += 0.5 * dt * source[it];
