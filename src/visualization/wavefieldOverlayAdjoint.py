...
for frame in np.arange(0,3500,10):
    imWave = plt.imshow(np.transpose(np.loadtxt("output/shot%i_adjoint/vx%i.txt" % (int(sys.argv[1]), frame))), animated=True,vmin=-2e-13, vmax=2e-13, aspect=1, cmap=customCMAP)
    ims.append([imMedium, imWave, plotBoundary[0]])


ani = animation.ArtistAnimation(fig, ims, interval=10, blit=True, repeat_delay=0)
ani.save('shotOverlay%iAdjoint.mp4' % int(sys.argv[1]), writer=writer)
